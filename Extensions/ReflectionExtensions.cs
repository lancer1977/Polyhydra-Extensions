...
namespace PolyhydraGames.Extensions
{
    public static class ReflectionExtensions
    {
        ...
        public static string FindByProperty<T>(this T obj, string name)
        {
            var fi = obj.GetType().GetRuntimeProperties().FirstOrDefault(f => f.Name == name);
            if (fi == null) throw new NullReferenceException($"Field {name} not found.");
            var value = fi.GetValue(obj).ToString();
            return value;
        }

        public static T FindByProperty<T>(this object obj, string name)
        {
            var fi = obj.GetType().GetRuntimeProperties().FirstOrDefault(f => f.Name == name);
            if (fi == null) throw new NullReferenceException($"Field {name} not found.");
            var value = (T)fi.GetValue(obj);
            return value;
        }
